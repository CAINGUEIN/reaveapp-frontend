...
export default {
  ...
  methods: {
    ...
    setupData() {
      ...
      this.select.bundle ? (this.bundle = this.select.bundle) : "";
      this.select.kit ? (this.kit = this.select.kit) : "";
      this.select.tags ? (this.tags = this.select.tags) : "";
    },
    async submit() {
      let body = {
        _id: this.select._id,
      ...
      console.log(body);
      let result = "";
      result = await EventServices.modifyItem(body);
