    ...
    return {
      chartData: {
        ...
        datasets: [
          {
            display: true,
            label: "visionScorePerMinute",
            fill: true,
            tension: 0.4,
            pointRadius: 0.25,
            pointHitRadius: 25,
            borderColor: "#CD6DFB",
            pointBackgroundColor: "#CD6DFB",
            borderWidth: 3,
            pointBorderColor: "#CD6DFB",
            backgroundColor: "",
            data: "",
          },
        ],
  ...
  methods: {
    ...
    lastMatch(indexSet) {
      ...
      for (let index = 0; index < indexSet; index++) {
        labels.push(index + 1);
        if (this.data1month[index]?.visionScorePerMinute !== undefined) {
          value.push(this.data1month[index].visionScorePerMinute);
        }
      }
    ...
    lastDayMatch(indexSet) {
      ...
      for (let index = 0; index < indexSet; index++) {
        ...
        for (let index2 = 0; index2 < this.data1month.length; index2++) {
          ...
          if (dateMatch === day) {
            dayData.push(match.visionScorePerMinute);
          }
        }
        labels.push(day);
    ...
    getGradient() {
      ...
      gradient.addColorStop(0, "rgba(205,109,251, 0.40)");
      gradient.addColorStop(0.25, "rgba(205,109,251, 0.20)");
