    ...
    <div>
      <div>
        <div class="flex items-center">
          <p class="text-[20px] text-White font-bold">Vision Avantage against lane opponent</p>
          <p class="text-[16px] font-normal ml-[5px]">(Vis/min delta)</p>
        </div>
        <div class="flex">
          <p
    ...
    return {
      chartData: {
        ...
        datasets: [
          {
            display: true,
            label: "visionScoreAdvantageLaneOpponent",
            fill: true,
            tension: 0.4,
            pointRadius: 0.25,
            pointHitRadius: 25,
            borderColor: "#7500BD",
            pointBackgroundColor: "#7500BD",
            borderWidth: 3,
            pointBorderColor: "#7500BD",
            backgroundColor: "",
            data: "",
          },
        ],
  ...
  methods: {
    ...
    lastMatch(indexSet) {
      ...
      for (let index = 0; index < indexSet; index++) {
        labels.push(index + 1);
        if (this.data1month[index]?.visionScoreAdvantageLaneOpponent !== undefined) {
          value.push(this.data1month[index].visionScoreAdvantageLaneOpponent);
        }
      }
    ...
    lastDayMatch(indexSet) {
      ...
      for (let index = 0; index < indexSet; index++) {
        ...
        for (let index2 = 0; index2 < this.data1month.length; index2++) {
          ...
          if (dateMatch === day) {
            dayData.push(match.visionScoreAdvantageLaneOpponent);
          }
        }
        labels.push(day);
    ...
    getGradient() {
      ...
      gradient.addColorStop(0, "rgba(117,0,189, 0.40)");
      gradient.addColorStop(0.25, "rgba(117,0,189, 0.20)");
